...
"""Single RPC entry point; forwards to validators with failover (RPC_BASE, RPC_BASE+1, ...).
   ...
   Polygon Edge has no built-in multiplexer; this is a custom proxy."""
...
def get_backend_ports():
    """Discover validator count from Node-* directories (Polygon Edge) and return list of RPC ports."""
...
def main():
    ...
    if not BACKEND_PORTS:
        print("No Node-* directories found. Run make init and make start first.", file=sys.stderr)
        sys.exit(1)
